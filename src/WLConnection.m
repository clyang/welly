...
- (void)login {
    ...
    while(i< maxAttempt) {
        ...
        for(j=20; j<25; ++j) {
            ...
            if([textLine containsString:@"請輸入"] || [textLine containsString:@"您的"] || [textLine containsString:@"代号:"] || [textLine containsString:@"Login"]) {
                onLoginScreen = YES;
    ...
    if(![_loginID isEqualToString:@""]){
        ...
        if (status == noErr) {
            while(i< maxAttempt) {
                ...
                for(j=20; j<25; ++j) {
                    textLine = [self getTerminalNthLine:j];
                    if([textLine containsString:@"密碼："] || [textLine containsString:@"密碼:"] || [textLine containsString:@"密碼]"] || [textLine containsString:@"密码:"] || [textLine containsString:@"Password"]) {
