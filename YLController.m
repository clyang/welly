	...
	for (int i = 0; i < [storage length]; ++i) {
		...
		if ((underline != preUnderline) || 
			...
			if (color == [config colorBlack])
				sprintf(tmp, "[0;%s%s30m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorRed])
				sprintf(tmp, "[0;%s%s31m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorGreen])
				sprintf(tmp, "[0;%s%s32m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorYellow])
				sprintf(tmp, "[0;%s%s33m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorBlue])
				sprintf(tmp, "[0;%s%s34m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorMagenta])
				sprintf(tmp, "[0;%s%s35m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorCyan])
				sprintf(tmp, "[0;%s%s36m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorWhite])
				sprintf(tmp, "[0;%s%s37m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorBlackHilite])
				sprintf(tmp, "[0;1;%s%s30m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorRedHilite])
				sprintf(tmp, "[0;1;%s%s31m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorGreenHilite])
				sprintf(tmp, "[0;1;%s%s32m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorYellowHilite])
				sprintf(tmp, "[0;1;%s%s33m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorBlueHilite])
				sprintf(tmp, "[0;1;%s%s34m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorMagentaHilite])
				sprintf(tmp, "[0;1;%s%s35m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorCyanHilite])
				sprintf(tmp, "[0;1;%s%s36m", underline ? "4;" : "", blink ? "5;" : "");
			else if (color == [config colorWhiteHilite])
				sprintf(tmp, "[0;1;%s%s37m", underline ? "4;" : "", blink ? "5;" : "");
