//
//  KOEditingCursorMoveHotspotHandler.m
//  Welly
//
...
- (id) init {
	[super init];
	if (!gMoveCursor)
		[KOEditingCursorMoveHotspotHandler initialize];
	return self;
}
...
- (void) update {
	if ([[_view frontMostTerminal] bbsState].state == BBSComposePost) {
		[_manager addTrackingAreaWithRect:[_view frame]
								 userInfo:[NSDictionary dictionaryWithObject:self forKey:KOMouseHandlerUserInfoName] 
								   cursor:gMoveCursor];
	}
}
